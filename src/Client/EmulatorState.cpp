...
void SystemIO::sound(int16_t frequency, uint16_t duration) {
    soundBuffer.push(std::pair<int16_t, uint16_t>(frequency, duration));
}
...
void EmulatorState::onTick(State *state, const uint32_t time) {
    ...
    std::optional<std::pair<int16_t, uint16_t>> s = sysio->nextSound();
    while (s != std::nullopt) {
        auto sound = *s;
